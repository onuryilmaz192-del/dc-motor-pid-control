...
Known İssues:  
Due to low pulse per rotation(spaces of disk) its hard to ensure an correct speed measuremen
in slower speeds and time periods so if you have an disk with an higher ppr please decrease the time reset period. 
With an one second time reset it is also takes longer for your motor to settle in your desired speed.
Due to overshooting(reading more pulses than reality) issues 
i added an simple if statement to ensure pulses readed within 10 milliseconds are not affecting the
actual pulse reading thats why in really high speeds(100 rotation per second) 
it will miss some pulses and will give you slower speed than it actually is.
